...
const axios = require('axios');  // Add this for video download
const app = express();
const PORT = process.env.PORT || 3000;

// Add CORS middleware
app.use(cors());  // Add this after creating app
// ─── Middleware ────────────────────────────────────────────────────────────────
...
// Add this after the other schemas (around line 50)
...
// Add this route after the quiz route (around line 550)

// STORIES PAGE
app.get('/stories', requireAuth, async (req, res) => {
  try {
    const stories = await Story.find().populate('userId', 'name email').sort({ createdAt: -1 });
    ...
    const rows = stories.map((story, i) => {
      ...
      return `
        <tr id="story-row-${story._id}">
          <td style="color:var(--muted);font-size:12px">${i + 1}</td>
          <td>
            <div class="avatar-cell">
              <div class="avatar">📖</div>
              <div>
                <div class="user-name" style="font-weight:600">${story.title || 'Untitled'}</div>
                <div style="font-size:11px;color:var(--muted)">${storyDate}</div>
              </div>
            </div>
          </td>
          <td>
            <div>
              <div class="user-name">${story.userId?.name || 'Unknown User'}</div>
              <div style="font-size:11px;color:var(--muted)">${story.userId?.email || 'No email'}</div>
            </div>
          </td>
          <td style="max-width:250px">
            <div style="font-size:13px;line-height:1.4">${truncateText(story.storyText, 80)}</div>
            ${story.videoUrl ? '<span style="font-size:10px;color:var(--accent3)"><i class="fas fa-video"></i> Has video</span>' : ''}
          </td>
          <td>
            <div style="display:flex;flex-wrap:wrap;gap:4px">
              ${story.character ? `<span class="pill" style="background:rgba(124,92,252,.1)"><i class="fas fa-user"></i> ${story.character}</span>` : ''}
              ${story.world ? `<span class="pill" style="background:rgba(232,93,138,.1)"><i class="fas fa-globe"></i> ${story.world}</span>` : ''}
              ${story.mood ? `<span class="pill" style="background:rgba(62,207,176,.1)"><i class="fas fa-smile"></i> ${story.mood}</span>` : ''}
              <span class="pill" style="background:rgba(255,255,255,.05)"><i class="fas fa-language"></i> ${story.language || 'english'}</span>
            </div>
          </td>
          <td>
            <div style="display:flex;gap:6px">
              <button class="btn-view" onclick="viewStory('${story._id}')" style="background:rgba(124,92,252,.1);color:var(--accent);border:1px solid rgba(124,92,252,.25);padding:5px 12px;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer">
                <i class="fas fa-eye"></i> View
              </button>
              <form class="del-form" action="/delete-story/${story._id}" method="POST" style="display:inline">
    ...
    <div class="layout">
      ${sidebarNav('stories')}
      <div class="main">
        ${header('Stories', 'Manage')}
        <div class="content">
          <div class="page-title fade-up">User Stories</div>
          <div class="page-subtitle fade-up delay-1">Browse and manage all stories created by users.</div>
          ...
          <div class="stats-grid fade-up delay-1">
            ...
            <div class="stat-card teal">
              ...
              <div class="stat-value">${stories.filter(s => s.videoUrl).length}</div>
              <div class="stat-label">Stories with Video</div>
              <div class="stat-trend up"><i class="fas fa-circle" style="font-size:7px"></i> Multimedia</div>
            </div>
            <div class="stat-card pink">
              <div class="stat-glow"></div>
              <div class="stat-icon"><i class="fas fa-chart-line"></i></div>
              <div class="stat-value">${Math.round(stories.reduce((acc, s) => acc + (s.storyText?.length || 0), 0) / (stories.length || 1) / 1000)}k</div>
              <div class="stat-label">Avg Words</div>
              <div class="stat-trend up"><i class="fas fa-arrow-up"></i> Per story</div>
            </div>
          </div>
          ...
          <div class="card fade-up delay-2">
            <div class="table-toolbar">
              <div class="search-box">
                ...
                <input id="storySearch" type="text" placeholder="Search by title, author, or content..."/>
              </div>
              <div class="table-count" id="tableCount">${stories.length} stories</div>
            </div>
            <div class="table-wrap card-body no-pad">
              <table>
                ...
                <tbody id="storiesBody">${rows}</tbody>
              </table>
            </div>
          </div>
        </div>
    ...
    <!-- Story View Modal -->
    <div id="storyModal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:10000;align-items:center;justify-content:center">
      <div style="background:var(--surface);border-radius:20px;max-width:800px;width:90%;max-height:85vh;overflow-y:auto;border:1px solid var(--border);position:relative">
        <div style="position:sticky;top:0;background:var(--surface);padding:20px 24px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
          <h3 id="modalTitle" style="font-family:var(--font-head);font-size:22px;margin:0">Story Details</h3>
          <button onclick="closeModal()" style="background:none;border:none;color:var(--muted);font-size:24px;cursor:pointer">&times;</button>
        </div>
        <div style="padding:24px" id="modalContent"></div>
      </div>
    </div>
    ...
    <script>
      ...
      if (searchInput) {
        searchInput.addEventListener('input', function() {
          ...
          document.querySelectorAll('#storiesBody tr').forEach(tr => {
            const text = tr.textContent.toLowerCase();
            tr.style.display = text.includes(q) ? '' : 'none';
          });
          const visibleCount = [...document.querySelectorAll('#storiesBody tr')].filter(r => r.style.display !== 'none').length;
          document.getElementById('tableCount').textContent = visibleCount + ' stories';
        });
      }
          ...
          if (data.success && data.story) {
            ...
            modalContent.innerHTML = \`
              <div style="margin-bottom:20px">
                <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid var(--border)">
                  <div><strong>Author:</strong> \${story.userId?.name || 'Unknown User'}</div>
                  <div><strong>Created:</strong> \${new Date(story.createdAt).toLocaleString()}</div>
                  <div><strong>Language:</strong> \${story.language || 'english'}</div>
                </div>
                
                <div style="margin-bottom:20px">
                  <h4 style="color:var(--accent);margin-bottom:10px">📝 Story</h4>
                  <div style="background:var(--surface2);padding:20px;border-radius:12px;line-height:1.6;white-space:pre-wrap">\${story.storyText || 'No content'}</div>
                </div>
                
                \${story.videoUrl ? \`
                  <div style="margin-bottom:20px">
                    <h4 style="color:var(--accent3);margin-bottom:10px">🎬 Video</h4>
                    <video controls style="width:100%;border-radius:12px;max-height:300px">
                      <source src="\${story.videoUrl}" type="video/mp4">
                      Your browser does not support the video tag.
                    </video>
                    <button onclick="downloadVideo('\${story.videoUrl}')" style="margin-top:10px;background:var(--accent);color:#fff;border:none;padding:8px 16px;border-radius:8px;cursor:pointer">
                ...
                <div style="margin-bottom:20px">
                  <h4 style="color:var(--accent2);margin-bottom:10px">✨ Story Details</h4>
                  <div style="display:flex;gap:12px;flex-wrap:wrap">
                    \${story.character ? \`<span class="pill" style="background:rgba(124,92,252,.2)"><i class="fas fa-user"></i> Character: \${story.character}</span>\` : ''}
                    \${story.world ? \`<span class="pill" style="background:rgba(232,93,138,.2)"><i class="fas fa-globe"></i> World: \${story.world}</span>\` : ''}
                    \${story.mood ? \`<span class="pill" style="background:rgba(62,207,176,.2)"><i class="fas fa-smile"></i> Mood: \${story.mood}</span>\` : ''}
                  </div>
                </div>
                
                \${story.panels && story.panels.length > 0 ? \`
                  <div>
                    <h4 style="color:var(--warning);margin-bottom:10px">🎨 Story Panels (\${story.panels.length})</h4>
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:16px">
                      \${story.panels.map((panel, idx) => \`
                        <div style="background:var(--surface2);border-radius:12px;overflow:hidden">
                          \${panel.imageUrl ? \`<img src="\${panel.imageUrl}" alt="Panel \${idx+1}" style="width:100%;height:150px;object-fit:cover">\` : ''}
                          <div style="padding:12px">
                            <strong>Panel \${idx+1}</strong>
                            <p style="font-size:12px;color:var(--muted);margin-top:6px">\${panel.text || ''}</p>
                          </div>
                        </div>
                      \`).join('')}
                    </div>
            ...
            showToast('Failed to load story details');
          }
        } catch (error) {
          console.error('Error loading story:', error);
          showToast('Error loading story details');
        }
      }
      ...
      async function downloadVideo(videoUrl) {
        try {
          window.location.href = '/api/download-video?videoUrl=' + encodeURIComponent(videoUrl);
    ...
    res.send(getShell('Stories', body, 'stories'));
  } catch (err) {
...
// DELETE STORY
app.post('/delete-story/:id', requireAuth, async (req, res) => {
  try {
    await Story.findByIdAndDelete(req.params.id);
    res.redirect('/stories');
  } catch (err) {
    console.error('Failed to delete story:', err);
    res.status(500).send('Failed to delete story');
  }
});

// ============================================
// STORY API ROUTES (Add these before app.listen)
// ============================================
...
app.get('/api/download-video', async (req, res) => {
  try {
    ...
    // You'll need axios for this - add at top: const axios = require('axios');
    // Download video from Cloudinary or any URL
...
// Also add the stories link to the sidebar navigation - update the sidebarNav function
// Find the sidebarNav function and add this new menu item under System or Main:

// In sidebarNav function, add this new nav item where you want it (around line 300-320):
// Under Main section or create a new section for Content:

// Replace the sidebarNav function with this updated version that includes Stories link:

// I'll provide the updated sidebarNav function that includes the Stories menu item
...
<div class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <!-- ADD STORIES LINK HERE - NEW SECTION -->
    <div class="nav-section-title">Content</div>
    <a href="/stories" class="nav-item ${active === 'stories' ? 'active' : ''}">
      <i class="fas fa-book-open"></i><span class="nav-label">Stories</span>
    </a>
